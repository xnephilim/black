...
Thank you for considering making contributions to Black and related repositories!

Black uses [Tendermint’s coding repo](https://github.com/tendermint/coding)
...
3. If you would like to contribute:
    1. If the issue is a proposal, ensure that the proposal has been accepted by the Black team.
    ...
    4. Follow standard Github best practices:
        1. Fork the repo
        2. Branch from the HEAD of `development`(For core developers working within the black repo, to ensure a
    ...
    8. Please make sure to run `make format` before every commit -
       ...
       There are CI tests built into the Black repository
...
Now `origin` refers to my fork and `upstream` refers to the black version.
So I can `git push -u origin master` to update my fork,
and make pull requests to black from there.
...
with [gogoproto](https://github.com/gogo/protobuf) to generate code for use in Black.
...
Black uses [GitHub Actions](https://github.com/features/actions) for automated testing.

## <span id="updating_doc">Updating Documentation</span>

If you open a PR on the Black repo, it is mandatory to update the relevant documentation in `/docs`. Please refer to
...
Black utilizes [semantic versioning](https://semver.org/).
...
**Note**: tharsis’s Black team currently cuts releases on a need to have basis.
