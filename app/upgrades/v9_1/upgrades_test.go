...
import (
	...
	v9 "github.com/xnephilim/black/v13/app/upgrades/v9_1"
	...
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				...
				coins := sdk.NewCoins(sdk.NewCoin("ablack", res))
				...
				suite.Require().Equal(balanceBefore.AmountOf("ablack"), sdk.NewDecFromInt(res))
			},
			true,
		},
		{ //nolint:dupl
		...
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if tc.expectedSuccess {
				for i := range v9.Accounts {
					...
					balance := suite.app.BankKeeper.GetBalance(suite.ctx, addr, "ablack")
