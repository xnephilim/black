...
func TestKeyring(t *testing.T) {
	...
	kr, err := keyring.New("black", keyring.BackendTest, dir, mockIn, TestCodec, EthSecp256k1Option())
	...
	info, mnemonic, err := kr.NewMnemonic("foo", keyring.English, blacktypes.BIP44HDPath, keyring.DefaultBIP39Passphrase, EthSecp256k1)
	require.NoError(t, err)
...
func TestDerivation(t *testing.T) {
	bz, err := EthSecp256k1.Derive()(mnemonic, keyring.DefaultBIP39Passphrase, blacktypes.BIP44HDPath)
	require.NoError(t, err)
	require.NotEmpty(t, bz)
	...
	path := MustParseDerivationPath(blacktypes.BIP44HDPath)
	account, err := wallet.Derive(path, false)
	require.NoError(t, err)
	...
	// Equality of Black implementation
	...
	// Equality of Eth and Black implementation
	...
	// Inequality of wrong derivation path of Eth and Black implementation
