...
var _ = Describe("Recovery: Performing an IBC Transfer", Ordered, func() {
	...
	Describe("from a non-authorized chain", func() {
		BeforeEach(func() {
			...
			err := s.BlackChain.App.(*app.Black).ClaimsKeeper.SetParams(s.BlackChain.GetContext(), params)
		...
		It("should transfer and not recover tokens", func() {
			...
			ibcOsmo := s.BlackChain.App.(*app.Black).BankKeeper.GetBalance(s.BlackChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
		...
		Describe("to the sender's own eth_secp256k1 account on Black (sender == recipient)", func() {
			...
			Context("with disabled recovery parameter", func() {
				BeforeEach(func() {
					...
					s.BlackChain.App.(*app.Black).RecoveryKeeper.SetParams(s.BlackChain.GetContext(), params) //nolint:errcheck
				})
			...
			Context("with a sender's claims record", func() {
				Context("without completed actions", func() {
					BeforeEach(func() {
						...
						s.BlackChain.App.(*app.Black).ClaimsKeeper.SetClaimsRecord(s.BlackChain.GetContext(), senderAcc, claim)

						// update the escrowed account balance to maintain the invariant
						err := testutil.FundModuleAccount(s.BlackChain.GetContext(), s.BlackChain.App.(*app.Black).BankKeeper, claimstypes.ModuleName, coins)
						s.Require().NoError(err)
					})
				...
				Context("with completed actions", func() {
					...
					BeforeEach(func() {
						amt := sdk.NewInt(int64(100))
						coins := sdk.NewCoins(sdk.NewCoin("ablack", sdk.NewInt(int64(75))))
						claim = claimstypes.NewClaimsRecord(amt)
					...
					It("should transfer tokens to the recipient and perform recovery", func() {
						...
						ibcBlack := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), receiverAcc, teststypes.AblackIbcdenom)
						Expect(ibcBlack).To(Equal(sdk.NewCoin(teststypes.AblackIbcdenom, coinBlack.Amount)))
					...
					It("should not claim/migrate/merge claims records", func() {
						...
						claimAfter, _ := s.BlackChain.App.(*app.Black).ClaimsKeeper.GetClaimsRecord(s.BlackChain.GetContext(), senderAcc)
			...
			Context("without a sender's claims record", func() {
				...
				When("recipient has additional ibc vouchers that originated from other chains", func() {
					...
					It("should not recover tokens that originated from other chains", func() {
						...
						ibcAtom := s.BlackChain.App.(*app.Black).BankKeeper.GetBalance(s.BlackChain.GetContext(), senderAcc, teststypes.UatomIbcdenom)
				...
				When("recipient has additional non-native ibc vouchers that originated from senders chains", func() {
					BeforeEach(func() {
						params := types.DefaultParams()
						params.EnableRecovery = false
						err := s.BlackChain.App.(*app.Black).RecoveryKeeper.SetParams(s.BlackChain.GetContext(), params)
						s.Require().NoError(err)
						...
						transferMsg := transfertypes.NewMsgTransfer(s.pathOsmosisBlack.EndpointA.ChannelConfig.PortID, s.pathOsmosisBlack.EndpointA.ChannelID, sdk.NewCoin(teststypes.UatomIbcdenom, sdk.NewInt(10)), sender, receiver, timeoutHeight, 0, "")
						...
						osmoIBCAtom := s.BlackChain.App.(*app.Black).BankKeeper.GetBalance(s.BlackChain.GetContext(), receiverAcc, teststypes.UatomOsmoIbcdenom)
