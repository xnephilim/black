...
// TestUpgrade tests if an Black node can be upgraded from one version to another.
...
// TestCLITxs executes different types of transactions against an Black node
	...
	}{
		{
			...
			cmd: func() (string, error) {
				return s.upgradeManager.CreateSubmitProposalExec(
					...
					5000,
					true,
					"--fees=5000000000ablack",
					"--gas-prices=50000ablack",
				)
			},
			expPass:   false,
