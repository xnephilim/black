...
import (
	...
	black "github.com/xnephilim/black/v13/types"
...
func LegacyEIP712Cmd() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(`$ %s debug legacy-eip712 tx.json --chain-id black_9000-1`, version.AppName),
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			chainID, err := black.ParseChainID(clientCtx.ChainID)
			if err != nil {
