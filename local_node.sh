...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	jq -r --arg amount_to_claim "$amount_to_claim" '.app_state["bank"]["balances"] += [{"address":"black15cvq3ljql6utxseh0zau9m8ve2j8erz89m5wkz","coins":[{"denom":"ablack", "amount":$amount_to_claim}]}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	for KEY in "${KEYS[@]}"; do
		black add-genesis-account "$KEY" 100000000000000000000000000ablack --keyring-backend $KEYRING --home "$HOMEDIR"
	done
	...
	black gentx "${KEYS[0]}" 1000000000000000000000ablack --keyring-backend $KEYRING --chain-id $CHAINID --home "$HOMEDIR"
	## In case you want to create multiple validators at genesis
	## 1. Back to `black keys add` step, init more keys
	## 2. Back to `black add-genesis-account` step, add balance for those
	## 3. Clone this ~/.black home directory into some others, let's say `~/.clonedBlack`
	## 4. Run `gentx` in each of those folders
	## 5. Copy the `gentx-*` folders under `~/.clonedBlack/config/gentx/` folders into the original `~/.black/config/gentx`
...
black start --metrics "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001ablack --json-rpc.api eth,txpool,personal,net,debug,web3 --api.enable --home "$HOMEDIR"
